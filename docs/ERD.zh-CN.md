...
> **状态**：活文档 —— 这里的任何决策都可以被推翻。最近更新 2026-04-16（A 组 UI 底线：同步状态徽章、Now View 节奏条、Ad-hoc 叠层、编辑会话通用化、Cycle 记号改 C1、日期格式表落地；B 组 Now View 结构：多 Chunk pill 行、Slot 三形态、Next Rail 视觉、去掉铁轨副视图、`CURRENT RAIL` chip、Now 顶栏 `Now` + Mono 副标；C 组 Today Track Shift 交互：Skipped 态改 hatching、桌面 hover 出动作栏、Active 主 CTA 改 tonal `Done`、统一 Shift 标签 sheet、去 bento 保留单条时间线；D 组 Cycle View 骨架：按 Template 堆叠 section、顶部 day header 唯一模板切换入口、Cycle pager picker、summary strip 聚合、`⤺ 撤销本次编辑` 按钮、hatching 三分语义、Backlog 变 split drawer；E 组 Template Editor：删 Save 按钮 / 首次进入 inline 引导、Radix 10 色 popover、顶栏 tab + 2px 色条 + dashed `+ 新建模板`、summary strip 聚合、card 式 Rail 行 + time pill popover picker、行间 gap chip `+ 填充 Rail`、`⋯` 行菜单放 Line 绑定 / check-in toggle；通知重审：删 OS push / Capacitor 通知 / 通知权限链路，Signal 塌缩为 `showInCheckin` 布尔，§5.6 / §5.7 合成一条主线 —— check-in 条 + 待决定队列是同一机制前后两个时态；F 组 缺失页面：Projects / Settings 共用 master-detail 形态，Review 单尺度瀑布 + 节奏匹配度热力图（状态染色 + hatching 三分语义），待决定队列按日期反序 + 每行 4 动作 + 侧栏 `·` 小点不显数字，Calendar 月历网格 + 点日弹 popover + 高级规则 drawer 四 section，新增 §5.9 Settings 定 5 section + 主题三档默认跟随系统 + i18n 语言在外观 / 时间制 + AI locale 在高级；G 组 设计语言：Terracotta CTA 用 `orange-9/10/11` 三档纯色不用渐变；No-Line Rule 明文白名单（装饰色条 + sticky hairline + focus ring）；Surface 四档 `sand-1..4` 取代 `border` 表达层级；圆角 token `sharp / sm / md / lg` = `0 / 6 / 10 / 16`；整站零 glassmorphism；非对称为默认布局）。
>
...
> 大三，三周内完成小组报告。
>
> 她建一个 Project "市场调研报告"（计划时间窗 2026-04-20 → 2026-05-10），拆出几个 Chunk："确定选题 20%"、"发问卷 50%"、"分析数据 80%"、"写报告初稿 100%"，另外还有几个没标里程碑百分比的附加事项（"整理参考文献"、"检查格式"）。她把 Chunk 逐个拖到周期视图对应天的某个时段 Slot 里（"分析数据"这个 Chunk 放在下周三 14:00–16:00 那格）。队友拖延导致"发问卷"晚了两天，她在那条 RailInstance 上做 Postpone Shift，其它 Chunk 不受干扰。最后标记 100% 的 Chunk 完成 → Project 自动归档。
...
- **Slot（槽位）**：某个 CycleDay 中某个 Rail 位置上的**规划内容容器**。可同时承载：
  ...
  - 有序的 `chunkIds` —— 属于某个 Project 的 Chunk 分配位置。
...
- **RailInstance**：某天某个 Rail 的执行态实例，携带 `status`（pending / active / done / skipped）、`plannedStart` / `plannedEnd`、可选的 `actualStart` / `actualEnd`、当天 override、(若有) 所属规划会话的 `sessionId`。
- **Shift（变道）**：对当天 Rail 实例的任意偏离。类型：延后（Postpone）、交换（Swap）、跳过（Skip）、缩短 / 延长（Resize）、替换内容（Replace）。Shift 是中性记录，可选附带原因标签（全局共享标签库，详见 §5.7）。
...
- **Line（长线 / 分组原语）**：DayRail 唯一的"多 Rail 分组"概念，呈现为一个连续谱：
  - 无 Phase 无 Chunk → **纯分组（标签化）**，仅用于归类（给若干 Rail、Ad-hoc Event 打上"工作""就医"等归属）。
  ...
  - 有 Chunk → **Project Line（项目型 / UI 称 "Project"）**，有限但可追加步骤。见下条 Chunk 详述。
  - **Line 与 Rail 一对多**：一个 Line 可驱动多个 Rail（小组作业拆成 5 个可独立 Shift 的 Rail）。
  - Phase / Chunk 可指向全部关联 Rail（整体演进）或特定 Rail（局部推进）。
  - Line 的拆解可手动或由 AI 协助（§6）。
- **Chunk（工作块）**：Project Line 的基本执行单位。属性：
  - `title`、`subItems`（内部 checklist，不单独排程）、`status`（pending / in\_progress / done）、`order`（可拖拽）。
  - **可选的** `milestonePercent`（0–100）：带百分比即"里程碑"，不带则是"附加事项"。Project 支持**无限追加** Chunk（含追加新的 milestonePercent）直至归档。
  - **Chunk 的完成是全局的**：一个 Chunk 最多排入一个 Slot（Chunk ↔ Slot 一对一；一个 Slot 可容纳多个 Chunk）。Slot 只是"我打算在这里推进"，在任一位置标记完成即 Chunk 全局 `done`，所有视图同步反映。
  - **Project 进度**：已完成 Chunks 中 `milestonePercent` 的**最大值**（不做加权总和；无 `milestonePercent` 的 Chunk 不参与进度计算，但计入"已完成事项数"）。
  - **归档触发**：`milestonePercent === 100` 的 Chunk 转为 `done` 时 Project 自动归档；也允许用户随时手动归档。归档后不支持解档；若想做"v2"，通过"复制新建"生成新 Project。
  - **计划时间窗**（Project Line 级）：可选 `plannedStart` / `plannedEnd`，作为软提示 —— 把 Chunk 排入窗口外的日期会警示但不阻止。
...
Slot        ──holds ────────▶ Chunk[]（0..N，一对多）
Chunk       ──assignedTo ───▶ Slot（0..1，最多一个）
Chunk       ──belongsTo ────▶ Line（Project 变体）
...
RailInstance──reflects ─────▶ Slot 内容（taskName + chunks）
...
```
  pending ──到达开始时间──▶ active ──到达结束时间──▶ done
     │                        │                       ▲
...
任何转移都会生成 Shift 记录。Shift 只是历史，不影响后续天数。
...
1. **当前 Rail 的 Slot 内容**（大字号，占主内容列）。大标题上方固定挂一枚 Mono 9px 大写 wide-letter-spacing 的小 chip —— 标签永远是 **`CURRENT RAIL`**（不写 `CURRENT CHUNK`，因为焦点的语义单位是 Rail；Chunk 是 Rail 里的具体动作）。大标题下方的呈现按 Slot 形态分三种：

   - **有 Chunk**：首个未完成 Chunk 的 title 作为**大标题**。多 Chunk 时大标题下方加一行小号副文 `第 1 / 共 3 个 chunk`；再下方是**紧凑 pill 行**，把其余 Chunk 按 `order` 列出 —— 每个 pill = 4px Project 色条 + Chunk title + 可选 `milestonePercent` 徽标；已完成的 Chunk 加**删除线**；点击 pill 跳到该 Chunk 详情（Project Line 详情内定位）。pill **不承载"标记完成"操作** —— 主动作按钮的"完成"永远作用于首个未完成 Chunk，一次点一条，避免"挑着完成"的操作陷阱。
   - **只有 `taskName`**：`taskName` 作为大标题；标题下挂一枚小 chip `Quick task`（JetBrains Mono 9px 大写 wide letter-spacing，和 ADHOC chip 同风格；色板用 slate step 3 底 + step 11 文字），明确"这不是 Project Chunk、不会在 Project 进度里留痕"。
   - **都没有**：大标题位显示巨大 `—`；下方一行克制副文 `这段时间空着。休息、思考，或随手做点什么。` **不**露"+ 添加"按钮 —— 当下添加内容不是 Now View 的职责，走 Today Track（§5.2）或 Cycle View（§5.3）。

   大标题旁（或下方，取决于视口宽度）展示**剩余时间**（Mono 大字 `45m`）+ 结束时钟（小号 Mono `ends 16:30`）+ 一条时间进度条（按 Rail 时长推进，**不是 Chunk 进度**）。
...
2. **下一个 Rail 卡片**：视觉和 Today Track 单条 Rail 行一致 —— 4px 左色条取该 Rail 自身 Radix step 9 色；**刻意不使用三等色（terracotta）** 作为"Next"强调色，三等色只给当前 Rail 和主动作按钮（§9.6）。卡内内容：
   ...
   - Rail 名称（中号字）。
   - Slot 预览摘要（小号副文，按 order 列出前 2 个 Chunk 的 title + 百分比，如 `热身 20%、有氧 50%`）。若 Slot 是 `taskName`-only，副文 = `taskName` + `Quick task` chip；若全空，副文显示 `—`。

3. **一对主动作按钮**：`完成 / 跳过`。点"完成"会把当前 Slot 里首个未完成的 Chunk 标为 done（Chunk 是全局状态，在任一 Slot 完成即全局完成）。若 Slot 只有 `taskName` 无 Chunk，"完成"对应 RailInstance → `done`。首页刻意不放"调整"入口 —— 需要改时间 / 换内容走 Today Track 的行级交互（§5.2），避免当下决策又多一步。
...
**右侧栏只承载 `Goal Context`**：当前 Slot 下挂的 Chunk 所属 Project / Line 的背景信息（进度、计划窗口、最近一次 Shift 摘要）。**刻意不放**：装饰性图片、激励引语、"今日士气 65%"式的进度数字、成就 / 连续天数计量。装饰与激励与 §1 的核心理念直接冲突。若 Slot 是 `taskName`-only 或全空，右侧栏显示一段中性提示（如 `这段没有长目标需要展开。慢下来没问题。`），不留空白压迫用户找事情填。

**主内容区刻意不设左侧"铁轨可视化"**（竖向圆点 / 竖轴图形等"今日形状"副视图）。今天的形状由下方节奏条统一承担 —— 同屏两套时间轴只会稀释注意力，且竖轴形态天然无法像节奏条那样按状态着色呈现节奏密度。

**底部节奏条**（rhythm bar）：今天的 Rail 轴线横贴在首屏底部，每段按 RailInstance 状态着色 —— `pending` slate step 6 / `active` primary step 9 / `done` sage step 9 / `skipped` slate step 4 斜纹。**不展示数字、不展示百分比**，即便今天全部完成也没有"圆满完成"提示 —— 中性的回顾留给专门的"**今日复盘**"（§5.8 中 Review 的日尺度）。节奏条的作用只是"一眼看到今天的形状"，不是打卡墙。

**首屏保留两个克制的 slot**：

- 顶部（条件出现）：未标记补录条（§5.7）—— 可忽略、不阻塞。
...
- **四态着色**：
  - `pending` slate step 2 底 + step 11 文字。
  - `active` primary step 3 底 + step 12 文字 + 左色条加粗到 6px（详见下文"Active Rail 的特殊形态"）。
  - `done` sage step 2 底 + step 11 文字、title 轻度删除线。
  - `skipped` **保留 Rail 自己的色条和圆点色**（都是 Rail step 9）+ Rail step 6 色的 2px 间距对角斜线 hatching 作为底纹 + `opacity-60` + title 斜体 + 删除线。**刻意不使用 tertiary terracotta** —— 按 §9.6，三等色只给当前 Rail、主动作按钮、Replace Shift 叠层；Skipped 通过"纹理 + 降饱和"而不是"换色相"传达，和 Ad-hoc 的 1.5px 虚线外框属同一套"纹理标记状态"语言。
- **Shift 痕迹**：若该实例今天内做过 Shift，行底部贴一条 inline 小字（`已延后 30min` / `已替换为 …` / `已跳过 · 天气`；"原因 chip"取自该 Shift 的首个 tag）；点击**就地展开**完整 Shift 历史（时间 + 类型 + 标签 + 备注），**不弹 modal**。

**Shift 的触发入口（桌面 / 移动分端差异化）**：

- **桌面（默认安静）**：Rail 行默认只有内容、无按钮。鼠标 hover 整行 → 右侧滑出一条 32px 高的动作栏，图标顺序 `Skip` / `Postpone` / `Replace` / `Add note`（Material `block` / `schedule` / `swap_horiz` / `sticky_note_2`）。键盘 Tab 到行时以 focus ring 呈现同一条动作栏（可访问性必须）。
- **移动**：左滑 = Skip、右滑 = Postpone（短程手势对应高频动作）；长按行弹出完整四项菜单作为兜底发现性。
- 四个 Shift 动作**统一走下文的 "Shift 标签 sheet"**；发现性差异只在触发手势上，后续流程一致。

**Active Rail 的特殊形态**：

- 底色、文字、色条按 `active` 态放大一档（primary step 3 / step 12 / 6px 色条）；右上角一枚 Mono `NOW` 小 pill（pulse 一个 primary step 9 小圆点）。
- **主 CTA = `✓ Done`**，tonal 样式：`bg-primary-container` + `text-on-primary-container`，**不使用 gradient**。gradient 按 §9.6 仅保留给更稀有的庆祝态（例："今日全部 Rail 完成"），而不是日常按钮。
- **次按钮 `⋯`** 弹出菜单 = `Skip` / `Postpone` / `Replace` / `Add note` 四项，**和非 Active 行的 hover 动作栏完全一致**，菜单密度可预期。
- **"提前完成"不是新概念**：点 `✓ Done` = `RailInstance.status → 'done'` + `actualEnd = now()`；若 `actualEnd < plannedEnd` 说明提前结束。Review 直接按差值聚合，**无需新增 `earlyFinish` 字段**。
- **Postpone 的子选择**（在 sheet 里二次展开）：`+15min` / `+30min` / `+60min` / `移到下一空档`（自动找今天下一段 gap）。**不做自由分钟数输入** —— 高频场景几乎都是 5 分钟倍数的跳格，离散档位反而更快。

**Shift 标签 sheet（四个 Shift 动作的统一收口）**：

用户触发任何 Shift 动作（Skip / Postpone / Replace / Add note）→ 浮出一个轻量化面板。桌面：右侧 320px 侧面板从外部滑入；移动：底部抽屉。版式：

```
跳过「Morning Run」
────────────────────
快速原因：  [🌧️ 天气]  [😴 太累]  [🤝 会议]  [⋯ 其它]
备注（可选）：________________________________
                                [取消]  [跳过]
```

- **快速原因 chip**：从全局标签库（§5.7）读出该 Rail 历史上**使用频次最高的 3 个 tag**；第 4 格固定为 `其它 → 打开全量标签选择 + 新建标签`。点 chip 即选中（可多选），再点主按钮一次完成 Shift + 打标签。
- **备注字段**映射到 `Shift.reason?: string`（可选文本）；textarea 内显灰色 placeholder `这次偏离的背景…（可跳过）`。
- **空 sheet 直接确认完全允许** —— 没有强制项、没有"请填原因"提示。把 Shift 作为**一次性反思窗口**而非审判，与 §1 / §9 的 "No guilt design" 承诺一致。
- Postpone / Replace 的 sheet 在主按钮之上多一行二次选择区：Postpone 显示四档（+15 / +30 / +60 / 下一空档），Replace 显示搜索框（可从 Template / Line 选择）+ 新建 Ad-hoc 入口。
- 默认焦点在主按钮（回车 = 确认）；数字键 1–4 快速选中对应 chip；Esc = 取消。
...
- **默认外观**：1.5px **虚线**外框 + slate step 2–3 极浅填色 + 默认中性灰（slate step 9）色条。**刻意不默认使用三等色（tertiary terracotta）** —— 三等色保留给 Replace Shift 的专属视觉。
...
- **Replace Shift 的特殊态**：若某条 Rail 今天被 Replace Shift 替换，原 Rail 行**保留位置但色条变虚线 + 去饱和**，行内附一行小字 `Replaced by: <新名称>`；被替换进来的内容以 Ad-hoc 叠层形式显示在同一位置。一眼能看到"原本该做什么"和"实际换成了什么"。
...
- 中段：**Top 3 Project inline 进度条**（8px rounded-full bar，每条左端 Project 色条 + Project 名小字 + 右端 Mono `12/20` 或百分比；选"最多 chunk 已排"的前 3）；超过 3 个收到 `+N 更多` → 点击弹 popover 列出全部 Project + 进度。
- 右端：`backlog N →` 按钮，N = 未排入 Slot 的 Chunk 总数；点击唤起下文的 Backlog 侧栏。
...
- **Section mini-day-header**（24px 高，跨 7 列）：
  - 每列显示星期缩写 + 日期数字（`Mon 12` / `Tue 13` / …）。
  - **该 Template 生效的列**：底色 Template step 2 + 文字 Template step 11（Radix "低对比背景 + 高对比文字"搭配）。
...
- 空 Slot（Template 生效 + 无内容）：虚线 border + 显眼的 `+ 添加`；hover 实化。点击弹 popover：`[新建 Chunk 到 Project]` / `[从已有 Chunk 挑选]` / `[快速文本 taskName]`。
- 有内容 Slot：顶部按 order 列出 Chunk pill（左 4px **Project** 色条 + 名称 + 可选 `milestonePercent`），底部若有 `taskName` 则以小号灰字附一行。点击 pill → 弹层 `[标记完成]` / `[移除此处分配]` / `[打开 Project]`。
...
- 批量操作：跨天复制 Chunk 分配、拖拽改期、整段跳过某 Rail。
...
- **抽屉内容**：Project / Chunk 列表（按 Project 分组、Chunk 可拖到 Slot），与 §5.5 的 Projects 独立视图互为补充（tab + 侧栏双入口）。
...
- **右栏（主轴）**：Rail 列表，时间升序自动排序。每张 Rail 卡片 = `border-l-4` 描边（取 `Rail.color` step 9）+ 左 4px 色条 + inline-edit 标题 + 可选 subtitle + 右侧 **time pill**（`08:00 → 10:00`, Mono）+ 色点 + `⋯` 行菜单。
  ...
  - **色点点击 → popover 2×5 Radix 色盘网格**（10 色 step 9，直径 28px，间距 12px），悬浮显示色名（`Sand` / `Sage` / `Clay` / `Slate` / `Moss` / `Amber` / `Teal` / `Pink` / `Mauve` / `Gray`），当前色带环状描边；选中即 commit，popover 关闭。
...
- **高级日历规则 drawer**：从右滑出，约 420 px。顶部提示 `规则按 Ad-hoc > 单日覆盖 > 范围覆盖 > 循环规则 > 星期规则 > 默认 的优先级生效`。分 4 section，每块带 `+ 新建` 入口：
  - **星期规则**（MVP 默认已填：工作日 → workday / 周末 → restday；可改）。
...
- 密集编辑场景（新建 Chunk、排 Phase、调进度、归档、重命名）在同屏完成，不反复 push。

**列表层**：

- 所有 Line 按类型 tab 分组：`Project` / `Habit` / 纯分组（MVP 首批只交付 Project；Habit 在 §12 roadmap 中）。
- 子 tab 切分 `Active` / `Archived`。
- 卡片展示：色条、名称、计划时间窗（若 Project 有 `plannedStart` / `plannedEnd`）、主进度条（Project = 已完成 Chunks 中 `milestonePercent` 的最大值）、已完成事项数 / 总事项数。
...
- Project：
  - Chunk 列表，可拖拽排序；每个 Chunk 行展示 title、`milestonePercent`（如有）、status、sub-item 完成度（如 `2/5`）、以及它被排入的 Slot（显示日期 + Rail 名）。
  - 未排入 Slot 的 Chunk 聚集在顶部"backlog"区；支持从详情层直接拖到 Cycle View 侧栏的目标 Slot。
  - 底部"+ 添加 Chunk"，可以直接添加附加事项（无 milestonePercent）或里程碑（设 milestonePercent）。
- 编辑入口：手动增删 Phase / Chunk；或点 `AI 协助拆解` 进入多步问答向导（§6）。
...
- **Signal 的唯一表现形式 = check-in 条**：用户**打开 App**（或 App 已在前台、新的 Rail 在当下结束）时，Today Track 顶部自动浮出：
  `☕ 《专注工作》09:00–11:00 已结束 · 完成 / 跳过 / Shift... / 忽略`
  - **命中条件**：`showInCheckin = true` 的 Rail，且已结束、本次打开周期内尚未标记。
  - **多条同时命中** → 折叠为一行 `3 条已结束的 Rail 待标记 ▾`，展开显示列表；单条处理后不自动折叠，列表保持连续可操作。
  - **按钮语义**：`完成` 和 `跳过` 直接写 status；`Shift...` 打开 §5.2 的 Shift 标签 sheet；`忽略` 仅把本次 check-in 露出移走，条目本身留在 §5.7 待决定队列里（≥ 24h 后的 check-in 周期里就不再主动露出）。
- **per-Rail 开关 `showInCheckin`**（默认 `true`）：Template Editor 行 `⋯` 菜单中勾选切换（见 §5.4）。关闭 = 这条 Rail 静默走完，不进 check-in 条也不进待决定队列（适合纯结构性 Rail，如"午休"—— 没什么要追踪的）。
- **不自动降级**：连续多天 `完成` / `跳过` 不会自动关掉 check-in；用户可以主动在 Rail 设置里关。静默替用户决定"不再 check-in"会从"安静"滑向"缺席"。

### 5.7 未标记补录：待决定队列

§5.6 的 check-in 条覆盖的是"刚结束"的时态。落到**更久以前**还没被标记的 Rail，走更克制的路径 —— 待决定队列。两者是同一套机制的前后两个时态：新鲜的主动浮出来请你决定一下，陈旧的静静待着等你有空再过。

**刻意不采用的设计：** "昨天的 Rail 没标记，今天不让操作"。这违背核心理念（偏离是一等操作，跳过无后果）。

**采用的设计：**

- 已结束**超过 24 小时**仍未标记 → 条目从 check-in 条沉入独立的**待决定队列**（独立页面、可批量处理）。用户可以挑某个闲下来的时刻一次性清掉。
- 队列永远不阻塞任何当前操作；Today Track / Cycle View / Template Editor 的流程互不干涉。
- 系统**不会自动**把未标记条目改写为 `skipped`。长期未决定就一直在队列里，这是用户的选择。
- 若某个 Rail 被连续多天漏标记，AI Observe（如已启用）温柔提示是否调整 / 归档。
- **"让它们都过去吧"批量操作**：队列堆得很多时，用户可以一键关闭**较旧的条目**（默认阈值：**超过 7 天**的项；可在 设置 → 高级 调整）。今天刚沉入队列的 Rail 仍留着 —— 它够新，值得用户决定一下。按钮文案：**"让它们都过去吧"**。确认弹窗：*"把超过 7 天的事都过去？过去的日子不会被改写，只是把这份待办缩短一些。"* 这些条目在历史里仍以"未标记"存在，只是不再被当作待决定事项提示。

**页面形态**：

- **顶栏**：标题 `待决定` + 合计行 `47 条 · 最早 Mar 12`；右上 `让它们都过去吧` tonal 按钮（灰态；右侧附 `(影响 31 条)` 计数，仅对 > 7 天项生效）。
- **主体**：按日期**反序**分组（最新在上），组标签 `Mar 14 (Fri) · 3 条`，组内逐行展示该日未标记的 RailInstance。
- **每行**：4px 左色条（`Rail.color` step 9）+ Rail 名 + 时间 `09:00–11:00` (Mono) + 4 个 inline 小按钮 `完成 / 跳过 / Shift... / 忽略`。按钮语义与 §5.6 check-in 条**完全一致**，心智迁移零成本。
- **无多选 / 无批量栏**：每条独立决定；批量操作只留"让它们都过去吧"一个通道。（设计意图：避免"一键全部完成"式的不假思索判定 —— 那会让 Shift 数据失真。）
- **空状态**：`一切清空了 · 下次漏标记的 Rail 会出现在这里`。
- **侧栏导航入口**：App 左侧 nav 的 `待决定` 项仅在队列 `> 0` 时露出一个 `·` 小点（**不显示数字**，避免"我有 47 件事没做"的焦虑数字）；鼠标悬浮 tooltip 给出具体条数。
...
- **节奏匹配度热力图**：行 = 本尺度出现过的 Rails（按出现频次倒序），列 = 本尺度的日期（日尺度时列 = 一整天的 Rails 时段；周 = 7 列；月 = 5–6 列周次）。每格按对应 RailInstance 状态染色：
  - `done` —— Rail 自身 `color` step 9 实色
  - `shifted` —— Rail 自身 step 7 淡色 + 格内箭头 `→`
  - `skipped` —— step 6 hatching（解锁了 C 组约定的三分语义）
  - `unmarked` —— step 4 hatching（"淡出"的尚未决定态）
  - 格子悬浮 → tooltip 显示 `{Rail 名} · {日期} · 状态 + 若 shifted 则附 tag`
...
1. **Decompose（拆解）**：把一个 Line 拆成 Phases（Habit）/ Chunks（Project，可带 milestonePercent）及关联 Rail 配置。
...
| 强调色 Terracotta       | `orange-9` / `orangeDark-9` | **仅**用于 Current Rail 标识、主行动按钮、Replace Shift。三档状态详见下方"Terracotta CTA"  |
...
- **Rail 用户色板** —— 采用 **[Radix Colors](https://www.radix-ui.com/colors)** 现成色板，不再手选 hex：
  ...
  10 个 Rail 颜色统一取下列 Radix scale 的 **step 9**（step 9 是 Radix 标注的"solid"纯色档，正好适合时间块填色）：
  | 角色       | Radix scale | 角色            | Radix scale |
  | -------- | ----------- | ------------- | ----------- |
  | Sand 沙   | `sand`      | Apricot 杏     | `amber`     |
  | Sage 鼠尾草 | `sage`      | Seafoam 海沫    | `teal`      |
  | Clay 陶土  | `brown`     | Dusty Rose 绛灰 | `pink`      |
  | Slate 石青 | `slate`     | Fog 雾紫        | `mauve`     |
  | Moss 苔绿  | `olive`     | Steel 钢       | `gray`      |
  能走**自然色系**（natural：sand / sage / olive / mauve / slate）就走，匹配 DayRail 低饱和基调。没有自然等价的（amber / teal / pink）用 Radix 的 saturated 版，step 9 也已经控制在不刺眼范围内。
...
| Shift 时间戳      | `14:28 · 已延后 30min`      | `14:28 · Postponed 30m`  | `{hour, minute}` + i18n 模板字符串（时长单位走 ICU）                                                    |
...
type Rail = {
  ...
  showInCheckin: boolean;      // true = 打开 App 时浮出 check-in 条（§5.6）；false = 静默走完，不进 check-in 条、也不进待决定队列
...
type Slot = {
  ...
  chunkIds: string[]; // 按 order 展示，0..N
};
...
type CalendarRule = {
  ...
  // weekday:    { templateId, weekdays: number[] }          (0 = 周日)
  // cycle:      { cycleLength, anchor: 'YYYY-MM-DD', mapping: string[] }  // mapping[i] = 第 i 天位置对应的 templateId
...
type Signal = {
  // 用户在 check-in 条或待决定队列上对某条 RailInstance 做出的显式响应（§5.6 / §5.7）。
...
type Phase = {
  id: string;
  name: string;
  targetRailIds?: string[]; // 留空 = 作用于 Line 下所有 Rail
...
- **v0.1（Web MVP）**：Template（workday / restday）、Rail CRUD（含时段互斥校验、时间轴左栏 + 焦点箭头）、Cycle / CycleDay / Slot、Now View、Cycle View（周期视图，含今日列高亮 + 单元格可编辑性设计）、Project / Chunk 最小集（Projects tab + Cycle View 侧栏双入口，手动增删 Chunk，里程碑自动归档）、Shift 基础（skip / postpone）、本地 localStorage（SQLite 后挪）、**第一天就有 i18n 脚手架（zh-CN + en）**。
- **v0.2**：Signal、Timeline 回顾、PWA 安装、Cycle View 只读复盘模式、标签库、SQLite 替换 localStorage。
- **v0.3**：Cycle View 规划模式（会话级撤销）、Template Calendar、Ad-hoc Event、待决定队列。
- **v0.4**：Habit Line（Phase 演进 + PhaseTransition 标记）、纯分组 Line、归档 + 复制新建、Chunk sub-item 完整化。
