...
> 大三，三周内完成小组报告。
>
> 她建一个 Project "市场调研报告"（计划时间窗 2026-04-20 → 2026-05-10），拆出几个 Chunk："确定选题 20%"、"发问卷 50%"、"分析数据 80%"、"写报告初稿 100%"，另外还有几个没标里程碑百分比的附加事项（"整理参考文献"、"检查格式"）。她把 Chunk 逐个拖到周期视图对应天的某个时段 Slot 里（"分析数据"这个 Chunk 放在下周三 14:00–16:00 那格）。队友拖延导致"发问卷"晚了两天，她在那条 RailInstance 上点"以后再说"（`status → deferred`），进入 Pending 队列，再在 Cycle View 里把它拖到周五 → plannedStart/End 重置、回到 `pending`；其它 Chunk 不受干扰。最后标记 100% 的 Chunk 完成 → Project 自动归档。
...
- **Signal 的唯一表现形式 = check-in 条**：用户**打开 App**（或 App 已在前台、新的 Rail 在当下结束）时，Today Track 顶部自动浮出：
  ...
  - **命中条件**：`status = 'pending'` 且 `plannedEnd < now`、且 `plannedEnd > now - 24h`（更老的走 §5.7 沉到 Pending 队列），且 `showInCheckin = true`。
  - **多条同时命中** → 折叠为一行 `3 条已结束的 Rail 待标记 ▾`，展开显示列表；单条处理后不自动折叠，列表保持连续可操作。
  - **按钮语义（与 §5.2 hover 动作栏完全一致）**：
    - `完成` → `status → done`
    - `以后再说` → `status → deferred`，落入 §5.7 Pending 队列
    - `归档` → `status → archived`，终态；循环 Rail 额外弹 3s toast `已归档今日的 <name>；明天的仍会生成`
...
- **重新排期**：用户在 **Cycle View 里把 Pending 项拖到某天的某个 Slot 位置** → `status` 回到 `pending` + `plannedStart/End` 重置到目标日 / Rail 的时间。拖拽是主要的"后悔药"入口，不在 §5.7 页面内做。
...
- **主体**：按日期**反序**分组（最新在上），组标签 `Mar 14 (Fri) · 3 条`，组内逐行展示该日未决定的 RailInstance。`deferred` 与 `pending(stale)` 在行上除了左侧图标略不同外视觉一致。
- **每行**：4px 左色条（`Rail.color` step 9）+ Rail 名 + 原计划时间 `09:00–11:00` (Mono) + 3 个 inline 小按钮 `完成 / 归档 / 拖到 Cycle →`。前两个就地写 status；第三个以 ghost 态提示"去 Cycle View 重新排期"，点击跳转。
