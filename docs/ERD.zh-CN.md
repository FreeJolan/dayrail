...
- **批量忽略较旧事项**：队列堆得很多时，用户可一键忽略超过 N 天的条目（默认阈值 **7 天**；可在 设置 → 高级 调整）。近期（≤ N 天）的 Rail 仍留在队列里，值得用户决定一下。**按钮文案就说清楚要做什么**：`忽略超过 7 天的事项`（不是修辞式的"让它们都过去吧"—— 隐晦的文案会让用户猜）。确认弹窗直接说明影响与代价：*"忽略超过 7 天仍未标记的 N 条事项？这些条目在历史里仍以 '未标记' 存在，不再出现在此队列。"* 历史不被改写；只是队列被缩短。
...
- **空状态**：`队列为空 · 未标记的 Rail 超过 24 小时后会出现在这里`。
