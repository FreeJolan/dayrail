...
### 故事 F：在 Claude Code 里决定，让它落进 DayRail · 阿哲（v0.13 起 · MCP 主路径）

> DayRail + Claude Code 订阅用户，桌面端日用。
>
> 晚上他在 Claude Code 里聊"想开始冥想"，聊到一半说"帮我加到 DayRail"。Claude Code 通过 DayRail 的 MCP **先读**了他的工作日模板，看到 7:30 有条"晨间梳洗"，于是提议：冥想排它前面 7:25、再加一条晚上 22:00，各 5 分钟，做成一个习惯。它把这套**写进 DayRail 的「待确认提案」—— 不是真实数据**。他桌面上开着的 DayRail 弹出这条提案：新建习惯「冥想」+ 两条 Rail + 绑定 + 今天起生效。他看一眼，觉得 22:00 太早，直接在提案上改成 22:30（或回 Claude Code 说"晚上改 22:30"，它重调一次 MCP）。点「确认」，整批一次落库，顶栏给他一个「撤销」。**全程他没手动建过任何一条 Rail。** 如果他当时想的是"这只是这几天试试"，在提案上把形态从「习惯」切成「临时任务」即可 —— 同一份诉求，换一种落地形态，不重新跟 AI 说。

### 故事 G：在别处聊完，贴回 DayRail · 阿哲（v0.13 起 · 粘贴兜底路径）

> 同一个人，但手边没有 Claude Code 的场景。
>
> 通勤路上他用手机跟 Claude（claude.ai）聊怎么重排这学期的复习节奏，Claude 给了他一段 Markdown 待办清单。**这段文字跟 DayRail 一点关系都没有，就是普通自然语言。** 到工位他整段复制，打开 DayRail，粘进「从 AI 粘贴」框。DayRail 用他配的 Claude 代理把这段话解析成几条提案，进**同一个**「待确认提案」区（和故事 F 共用暂存区 + 确认引擎，只是"东西怎么进来"不同）。他调一下、确认、落库。
>
> 两条故事共同划定本轮 AI 的精确边界：**关掉「我已经决定要做 X」到「DayRail 里把 X 设置对了」之间的多步手工活**。不是做通用 AI 助手，不是替代 Claude Code 的对话，就关这一条缝。
...
- **非空 Slot**（≥ 1 个 task）：
  ...
  - **「未归属」行（off-rail row）+ 其拖拽边界（v0.12.5 加固）**：当某天的某条 Rail 不在任何 section 渲染（切了模板 / rail 失活），原本排在它上面的 task（`slot.railId` 仍指向那条 rail）不再静默隐身，而是 derive 进该天底部一行「未归属」展示（`cycleFromStore` 的 `offRailByDate`，合成 `railId = '__offrail__'` —— **纯渲染值，永不落库**）。这一行**只作拖拽源、绝不作放置目标**：把 off-rail pill 拖回某条真实 Rail 即重新排期。
    - **v0.12.5 修一个静默数据损坏 bug**（外部 review 发现）：off-rail 行的 pill 因被包在 `SortableContext` 内而注册为 droppable，偏好 pill 的碰撞检测可能把"放置目标"解析到 off-rail pill 自身/兄弟；`handleDragEnd` 据此把 `slot.railId` 写成合成值 `'__offrail__'`，**抹掉 task 真实 rail 关联**，且因结果仍 derive 回 off-rail 行而**完全不可见**（直到原 Rail 那天重新激活、task 却回不去）。`scheduleTaskToRail` 不校验 railId，`sameSlot` 守卫拿真实 railId 与 `'__offrail__'` 比也不相等、反而当成真实移动放行。修法：`handleDragEnd` 在最终目标 `railId === OFF_RAIL_RAIL_ID` 时**直接回滚不写**（mirror 已清，视觉自动还原）；`OFF_RAIL_RAIL_ID` 提为共享常量（`lib/dndContext.ts`），让产生方（CycleSection）与守卫方（App）同源不分叉。task 与 occurrence pill 两条分支同修。**不加** store 层守卫：`'__offrail__'` 是纯前端合成值、仅拖拽这一条路径能产生，drag-end 是所有拖拽写入的唯一汇聚点，守在那儿即完整且分层正确。
...
### 6.7 AI 意图拆解 · 暂存区落库模型（v0.13 起 · 设计锁定 2026-05-24）

> 对应故事 F / G。这一节是 §6.1 **Decompose 场景的反转实装**：原 §6.1 设想的"DayRail 内多步问答向导"被否，改为"拆解发生在外部（Claude Code）或粘贴进来，DayRail 只当**暂存 + 落库的靶子**"。MCP 的桌面实现细节见 §15.10。

#### 6.7.0 受众前提（先写死，再设计）

**这一节的所有设计只对准一种用户：Claude Code + Claude 订阅用户，桌面端日用。** 这不是傲慢，是 scope 的刹车 —— 我们反复犯过"为照顾各种用户而加分支，把要精确解决的那一个场景稀释成四不像"的错（这是我们固化下来的设计原则之一）。据此**直接砍掉**：多 provider 适配层、fallback chain UI、"没有 AI 的用户"引导 / 教程 / 空状态文案（都不在受众内，且与 ROADMAP 既有 ❌ 一致）。

#### 6.7.1 解决的场景（一句话）

我**已经想清楚要做什么**了（通常在跟 Claude 聊的过程中），但把它变成 DayRail 里正确的样子要手动点一长串：建习惯、建两条 Rail、各自绑定、排时间……**「我决定了」与「DayRail 里设置好了」之间那段多步手工活，就是痛点。本轮只关这条缝。**

#### 6.7.2 统一模型 · 一条流水线

无论出主意的 AI 在内（粘贴路径的内部 AI）还是在外（Claude Code via MCP），都收敛到同一条流水线：

```
自然语言意图
   │  （粘贴 → 内部 AI 解析 / MCP → Claude Code 直接吐结构化）
   ▼
一组「意图 spec + 形态」的提案  →  进【本地暂存区 · 待确认提案】
   │  （我手动调 / 内部 AI 单次微调 / 回 Claude Code 重调；可切形态）
   ▼
确定性投影器 → 一组「只增」操作  →  确认后整批落 Y.Doc（一个 Edit Session · 可一键撤销）
```

**提案的数据形态 =「意图 spec + 形态」**：

- **意图 spec**：规范化的语义内容，与 DayRail 实体解耦。形如 `{ 标题/活动, 单次时长?, 时间点[], 频率, 时间跨度, 备注? }`。这是 AI 真正负责产出的东西。
- **形态（shape）**：`habit`（长期习惯）/ `task`（任务 + 可选切分 occurrence）/ `adhoc`（排到具体日期的临时项）。投影器按 (意图 spec, 形态) 渲染出对应的实体图。
- **形态切换是确定性重投影、不重新调 AI** —— 故事 F 里"把习惯切成临时任务"就是拿同一份 spec 换一个形态投影。这把"AI 要输出的 schema"压到最小（只描述语义），把"产出哪些实体"的复杂度留在可单测的确定性投影器里。

#### 6.7.3 暂存区（staging tray）

- **是一个真实、可停留的界面**，不是一闪而过的弹窗 —— 尤其 MCP 路径，外部 AI 塞完提案时我可能没在看 DayRail，提案要留在那等我回来处理。
- **本地持久化，不进 Y.Doc 同步流，不归档**。理由三条：(1) 提案是"当场处理掉"的临时待办，跨设备价值低；(2) 与 §6.1「AI 输出 ephemeral、不进同步流」心智一致；(3) 提案**自包含、整条进整条出**，吃 §7.9 的元数据—数据生命周期错位那条教训 —— 不把提案数据和它的门控状态拆到两个生命周期不同的介质里。
- **生命周期**：进来（粘贴解析 / MCP 写入）→ 调整（手动改 spec 字段 / 切形态 / 内部 AI 单次微调）→ **确认**（投影 + 落库，提案被消费）或 **丢弃**。无状态机、无指派、无提案历史 / 版本 —— 它不是工作流系统。

#### 6.7.4 确认引擎 · 只增

- **只增，不改不删**。投影器只产出 create / insert 操作，映射现有 writer：`createRail` / `upsertHabitBinding` / `createTask` / `scheduleTaskOccurrence` / `overrideCycleDay` 等。`effectiveFrom` 默认今天、可在提案上改（接 §10.5 revision 模型）。
- **为什么不做改 / 删**：改撞 §10.5 effective-from revision 链、删撞 §10.3 purge 流程，是 blast radius 最大的两处；且 AI 改错的纠错成本 > 手动改的成本。改 / 删让用户手动做，效率和安全都更好。**显式不做。**
- **确认 = 一个 Edit Session**：整批 ops 在一个 `Y.transact(origin = sessionId)` 里落库，复用 §5.3.1 的 `Y.UndoManager` + Reason-toast 风格「撤销」。于是有**两层安全网**：review 是事前一层，一键撤销是事后一层。

#### 6.7.5 两条入口

- **粘贴（先落地 · 零 plumbing）**：Settings → AI 或 Tasks 区一个「从 AI 粘贴」入口。外部 Claude 吐的是**"跟 DayRail 无关的"纯自然语言 / Markdown**（不需要教外部 AI 任何 DayRail 格式）；粘进来后由**内部 AI 解析**成意图 spec。内部 AI 走现有 BYOK endpoint（§6.6）—— 对 Claude Code 订阅用户而言自然就是其 Claude 代理（`claude-code-router` / CLIProxyAPI 这类本机 OpenAI-compat 网关），所以"内部 AI"也是 Claude 质量，不是弱模型。
- **MCP（桌面专属 · 紧接着接）**：Claude Code 直接调结构化工具塞提案，**DayRail 这边不做任何解析**。详见 §15.10。

#### 6.7.6 schema drift 规避（别重蹈 v0.8.2 的坑）

v0.8.2 的教训是"让模型在散文里吐 canonical JSON、再正则抠出来"会持续 drift。本轮**两条路径都不走那个机制**：

- **粘贴路径**：用 AI SDK 的 `generateObject` / 工具调用 + Zod 约束 **intent spec**（一个小而闭合的 schema，远比当年开放式 observation schema 易学）。对 endpoint 做**能力门**：不支持结构化输出 / 工具调用的端点，提示降级（先只支持手动建，或提示换支持的端点）。
- **MCP 路径**：Claude Code（强模型）走原生结构化 tool-calling 直接填好字段，DayRail 不解析 —— 从机制上就没有"抠 JSON"这一步。

#### 6.7.7 刻意不做（防四不像 · 本节的硬边界）

- ❌ **多 provider 适配 / fallback chain / provider 市场** —— 假设受众就是 Claude Code 用户，不为不存在的受众加分支。
- ❌ **通用自然语言命令行 / 万能 in-app 助手** —— 本功能只干一件事：把已拆解的计划变成只增的实体、经我确认。
- ❌ **AI 改 / 删** —— 见 §6.7.4。
- ❌ **暂存区做成工作流系统**（状态机 / 指派 / 提案历史 / 版本）—— 它只是个待确认托盘。
- ❌ **in-app 内部 AI 做多轮聊天** —— 真要来回讨论我本来就在 Claude Code 里，不在 DayRail 里重造一个 Claude Code；内部 AI 只做单次微调。
- ❌ **"没有 AI 的用户"的引导卡 / 空状态教程 / 新手流** —— 不在受众内。
...
**v0.12.x 设计修正 · 失败判定从「push 时长」改为两轴模型（产品方案已定 · 待批准实现）**

> 上面的时长阶梯（时长可见 / 永不阻塞）**保留**，只是它的**判定输入**有缺陷，下面修正；阶梯改挂到"真失败"上。

**dogfood 暴露的缺陷**：分类器 `classifySyncStatus` 只看 `lastSuccessPushIso`（距上次成功 **push** 时长）。但 push 只在**有改动**时触发（8s debounce / 生命周期 / 手动），**没有闲置周期 push**（heartbeat 只搭在成功 push 之后）。后果：

- **本地与远端完全一致、但闲置 >1h**（尤其升级重启后还没改东西）→ `lastSuccessPushIso` 陈旧 → **误报「同步断开」**，尽管 0 待传、毫无问题。
- 反过来，**push 新鲜但 pull 探针静默失败、已落后远端**这个**真**风险，它**不报**（push 时间戳是新的）。
- 即"该叫的不叫、不该叫的乱叫"。

**重构原则 —— 谎报「断开」本身就是 false signal**：§7.10 的立论是"sync 是信任问题"。持续误报「断开」会训练用户**忽略**指示器，真断开时没人信 —— 亲手摧毁护栏要保护的信任。故告警词（"断开/失败"）**只留给真失败**，绝不被"闲置"触发。

**两轴模型** —— 角标只回答用户真正关心的两件事：

| 轴 | 用户的问题 | 信号 |
|---|---|---|
| Push | 我的改动**存上去**了吗？ | `pendingCount` + push 是否在**报错** |
| Pull | 我看到的是**最新**吗？ | 最近是否有**成功 pull/探针** + pull 是否在报错 |

**修正后状态机**（角标取第一个命中）：

| # | 状态 | 触发 | 主文案 | 色 |
|---|---|---|---|---|
| 1 | 未连接 | `!connected` | 本地 | 灰 |
| 2 | 同步中 | 有 push/pull 在飞 | 同步中 | 中性 |
| 3 | 离线 | 无网络 | 离线 | 暖弱 |
| 4 | 改动没传上去 | `pending>0` 且 push 持续报错 | {N} 个改动没传上去（hover 带时长） | 暖强 |
| 5 | 连不上云端 | pull/探针持续失败 | 连不上云端（hover 带时长） | 暖 |
| 6 | 待传 | `pending>0`，push 没报错 | 未同步 · {N} | 暖弱 |
| 7 | 已同步 | `pending==0` 且 最近成功 pull 且 `sessionRoundTripDone` | 已同步（hover：刚同步过 · N 分钟前） | OK |
| 8 | 检查中 | `pending==0` 但 pull 新鲜度未确认（刚重启 / 探针未回） | 检查中 | 灰弱 |

**「同步断开」退役**，拆成语义精确三块：#4 改动没传上去（数据真有风险）/ #5 连不上云端（真连不上）/ #8 检查中（重启后短暂未确认 · **不告警**）。时长阶梯（1 天 / 3 天）保留，挂到 #4/#5。

**"已同步"门槛收紧 = 防 false-OK 加强**：报「已同步」必须 `pending==0`（我的都上去了）+ 最近成功 pull（没落后远端）+ `sessionRoundTripDone`（本会话真同步过，堵 wipe / 空库假 OK）。以前 push 新鲜就敢隐含 OK；现在多了 pull 新鲜度背书 —— 比旧版**更**防假 OK。#7 hover 显式写"刚同步过 · N 分钟前"，让"最新性"诚实可见（不假装毫秒实时）。

**默认旋钮**（可调）：pull 新鲜窗口 = 10 min（2× 探针间隔 · 容一次漏拍）；失败阶梯阈值沿用 1 天 / 3 天。

**范围**：纯展示层 —— 改 `classifySyncStatus`（`packages/core/src/syncStatus.ts`）+ `describeSyncStatus`（`SideNav.tsx`），把**已存在但没用的** `lastSuccessAt.pull` + `recentAttempts`（本节上面的 schema）接进分类输入。无 schema / `.dryj` 改动。设置里的「已连接」(consent 标志) 不动 —— 它本就该与"健康度"分离。
...
#### v0.12.3 · 切分快速添加：数字识别为进度

dogfood 反馈：建一个「纯里程碑进度」切分（只有 `percent`、没名字）原本要两步——底部添加框留空回车建一条 → 再在那行的 `%` 框里填数字。添加框是「步骤名」导向的，没法一步建成。

v0.12.3 给添加框加一条轻量识别：输入**纯整数且 ≤ 100**（即落在 percent 合法区间 0–100）→ 直接建一个该 `percent` 的**无名**切分；**超出范围**（150 / 2024 …）或含非数字字符 → 仍当步骤名（数字命名的步骤不丢，如「365 天打卡」）；留空 → 无名空切分（不变）。
...
### 15.10 MCP server（v0.13 起 · 桌面专属 · 读 + 写工具 + 暂存区桥接）

> §6.7 统一模型的 MCP 入口实装。对应故事 F。

**为什么是桌面专属**：MCP server 需要一个常驻进程对外暴露工具；Tauri 的 Rust 进程天然就是。PWA 没有持久进程，做不了 —— 所以 MCP 是桌面专属，**粘贴（§6.7.5）是 PWA 兜底**。这与 §15.1「PWA 不下线、但日用推荐桌面端」一致。

**进程形态**：MCP server 跑在桌面端 Tauri/Rust 进程内（stdio transport 对接 Claude Code / Claude Desktop；或本机 loopback HTTP/SSE）。**默认关闭**，Settings → AI（或 → 同步旁）一个 toggle 打开；只监听本机，不对外。

**读工具（read tools）—— 让外部 Claude 拿到我的真实节奏**：

- `get_templates` / `get_rails_for_date(date)`：某天/某模板下有哪些 Rail、几点。
- `get_habits`：现有习惯 + 绑定。
- `get_schedule(dateRange)`：一段日期内的排期（occurrence / task slot）。
- `get_recent_reflections(n)`：最近 n 条日反思（供 Claude Code 带上下文聊 / 提议）。

读工具是 MCP 相对粘贴的**核心增量**：Claude Code 有了我的真实状态，提议才 grounded（冥想贴在已有 7:30 Rail 前面、避开常跳过的周四），而不是瞎拍时间。

**写工具（write tools）—— 只塞暂存区，不直接落库**：

- `propose(items)` / `propose_habit(...)` / `propose_task(...)`：Claude Code 走结构化 tool-calling 填好 §6.7.2 的「意图 spec + 形态」，工具把提案**写进本地暂存区**，**不碰真实数据**。返回类似「已提交到 DayRail 暂存区，请在应用内确认」。
- 调整也可经 MCP：Claude Code 调更新工具改暂存区里某条提案（对应故事 F"晚上改 22:30"）。

**桥接机制**：Rust 侧 MCP handler 收到 `propose*` → Tauri `emit` 一个事件给 webview → 前端 listener 把提案写入本地暂存区 store → 暂存区 UI 出现待确认提案。**确认动作仍在 DayRail UI 内人工完成**（MCP 工具本身不触发落库）。

**隐私边界**：读工具把我自己的数据喂给我自己的 Claude，且**要我主动在 Settings 里开这个 MCP server 才发生** —— 与 §6.5 / §7.1 的 BYOK 心智一致。server 仅本机 loopback，不暴露任何凭证读取工具。
