...
(**v0.2 decision: every Cycle View planning action is immediate-apply** — CycleDay template switches write `calendar-rule.upserted` / `calendar-rule.removed`, Slot drags write `task.scheduled` / `task.unscheduled` — none of them enter the §5.3.1 Edit Session. The top-bar `⤺ Undo this edit · N` button is deferred to v0.3; we'll revisit whether to bring it back once we see real usage patterns.)
...
- **The top-level day header is also the sole CycleDay template-switch entry**: click any date cell → popover listing every template the user has created, each row as `radio + Template color bar + name`, with `+ New template` at the bottom. Selecting one emits a `calendar-rule.upserted` event (`kind: 'single-date'`, id deduplicated via `cr-single-{date}` so flipping the same day repeatedly just updates in place); "Restore default" emits `calendar-rule.removed` and falls back to §5.4's weekday heuristic. Every section's mini-header and cells redraw immediately (old section's column turns hatched, new section's column lights up). The section mini-header itself stays **read-only** — one action gets one entry point, never two.
...
- Entering any "deep-edit" view (v0.2 scope: **Template Editor only**; Cycle View's session is deferred to v0.3; the mechanism extends to Line editing, Calendar rule editing, etc. in later versions) opens an **implicit session**.
...
- **Cycle View** (v0.3+): the planning session will cover every RailInstance override and Slot content edit made inside the view. **In v0.2 scope, Cycle View actions are immediate-apply** — CalendarRule emits single-date events, Slot drags go through `task.scheduled` — and do not enter the session. Mistaken actions are walked back one at a time via the Slot popover's "Remove assignment" button and the CycleDay popover's "Restore default" button.
...
type CalendarRule = {
  ...
  // v0.2 implemented scope: only `single-date` (the "flip a day's template in Cycle View" use case).
  //   - id convention: `cr-single-{date}` (so flipping the same day repeatedly is an update, not a new row)
  //   - priority: 100 by default (beats the implicit 0 from the weekday heuristic)
  //   - events: `calendar-rule.upserted` (payload = { id, kind, value, priority }) / `calendar-rule.removed` (payload = { id })
  // Other kinds' shapes are preserved; the weekday / cycle / date-range resolver + UI lands in v0.3 Calendar view.
};
